...
class EmojiTest < TestCase
  ...
  test "unicode_aliases includes alternate position of VARIATION_SELECTOR_16" do
    emoji = Gemoji.find_by_unicode("\u{0031}\u{fe0f}\u{20e3}")
  ...
  test "unicode_aliases doesn't necessarily include form without VARIATION_SELECTOR_16" do
    emoji = Gemoji.find_by_unicode("\u{00a9}\u{fe0f}")
    assert_equal ["\u{00a9}\u{fe0f}"], emoji.unicode_aliases
  end

  test "emojis have tags" do
    emoji = Gemoji.find_by_alias('smile')
  ...
  test "emojis have valid names" do
    invalid = Gemoji.all.reject { |emoji| emoji.name =~ /^[\w\+\-]+$/ }
    assert_equal [], invalid, "some emoji have invalid names"
  end

  test "custom emojis" do
    custom = Gemoji.all.select(&:custom?)
  ...
  test "custom emoji names" do
    custom_names = Gemoji.all.select(&:custom?).map(&:name)
    assert custom_names.include?("shipit")
    assert !custom_names.include?("+1")
  end

  test "create" do
    emoji = Gemoji.create("music") do |char|
    ...
    begin
      assert_equal emoji, Gemoji.all.last
      assert_equal emoji, Gemoji.find_by_alias("music")
      assert_equal emoji, Gemoji.find_by_unicode("\u{266a}")
      assert_equal emoji, Gemoji.find_by_unicode("\u{266b}")
  ...
  test "edit" do
    emoji = Gemoji.find_by_alias("weary")

    emoji = Gemoji.edit_emoji(emoji) do |char|
    ...
    begin
      assert_equal emoji, Gemoji.find_by_alias("weary")
      assert_equal emoji, Gemoji.find_by_alias("whining")
      assert_equal emoji, Gemoji.find_by_unicode("\u{1f629}")
      assert_equal emoji, Gemoji.find_by_unicode("\u{1f629}\u{266a}")
